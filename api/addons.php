...
<?php if ($type == 'video-fotorama') {
    ...
    if (count($video_home)) { ?>
<div id="fotorama-videos" data-width="100%" data-thumbmargin="10" data-height="260" data-fit="cover"
...
<?php if ($type == 'video-select') {
    $video_home = $d->rawQuery("select video, id, name$lang from #_news where type = ?  and find_in_set('hienthi',status) order by numb, id desc", array('video'));
    if (count($video_home)) { ?>
<div class="video-main">
    <iframe width="100%" height="320" src="//www.youtube.com/embed/<?= $func->getYoutube($video_home[0]['video']) ?>"
...
<div class="fb-page" data-href="<?= $optsetting['fanpage'] ?>" data-tabs="timeline" data-width="500" data-height="400"
