...
if ($cmd == 'add-cart' && $id > 0) {
	...
	if (!empty($_SESSION['cart'])) {
		...
		for ($i = 0; $i < $max; $i++) {
			if ($code == $_SESSION['cart'][$i]['code']) {
				$regular_price = $func->formatMoney($cart->sumPrice($_SESSION['cart'][$i]['productid'], $_SESSION['cart'][$i]['color'], $_SESSION['cart'][$i]['size'], $quantity));
	...
	$data = array('regularPrice' => $regular_price, 'tempText' => $tempText, 'totalText' => $totalText);
