...
function exportExcel()
{
	...
	if (isset($_POST['exportExcel'])) {
		...
		foreach ($array_columns as $k => $v) {
			...
			} else if ($k == 'regular_price' || $k == 'old_price') {
				$PHPExcel->getActiveSheet()->getColumnDimension($alphas[$i])->setWidth(10);
			} else if ($k == 'titlevi' || $k == 'keywordsvi' || $k == 'descriptionvi') {
		...
		for ($i = 0; $i < count($product); $i++) {
			...
			foreach ($array_columns as $k => $v) {
				if ($k == 'id_list') {
					...
					$namecat = $d->rawQueryOne("select * from #_seo where id_parent = ? limit 0,1", array($product[$i]['id']));
					$datacell = (!empty($namecat[$k])) ? $namecat[$k] : '';
				} else {
