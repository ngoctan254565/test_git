...
function editMan()
{
	global $d, $func, $strUrl, $curPage, $item, $gallery, $tt, $type, $com, $act, $configBase;
	...
	if (empty($id)) {
		...
		$tt = $d->rawQuery("select * from #_product_sale where id_parent = " . $id . " order by id asc");
	}
}
...
function saveMan()
{
	...
	if ($data) {
		...
		if (isset($config['product'][$type]['sale']) && $config['product'][$type]['sale'] == true) {
			if (isset($_POST['id_sale']) && ($_POST['id_sale'] != '')) $data['id_sale'] = implode(",", $_POST['id_sale']);
			else $data['id_sale'] = "";
		}

		$data['regular_price'] = (isset($data['regular_price']) && $data['regular_price'] != '') ? str_replace(",", "", $data['regular_price']) : 0;
		$data['old_price'] = (isset($data['old_price']) && $data['old_price'] != '') ? str_replace(",", "", $data['old_price']) : 0;
		$data['discount'] = (isset($data['discount']) && $data['discount'] != '') ? $data['discount'] : 0;

		$data['type'] = $type;
	}

	if (isset($_POST['id_color']) && ($_POST['id_color'] != '')) $data['id_color'] = implode(",", $_POST['id_color']);

	else $data['id_color'] = "";

	if (isset($_POST['id_size']) && ($_POST['id_size'] != '')) $data['id_size'] = implode(",", $_POST['id_size']);
	...
	if (!empty($data['old_price']) && !$func->isNumber($data['old_price'])) {
	...
	if ($id && $act != 'save_copy') {
		...
		if ($d->update('product', $data)) {
			...
			if (!empty($_POST['price_attrs'])) {

				$price_attrs = array();

				$price_attrs['id_parent'] = $id;

				$d->rawQuery("delete from #_product_sale where id_parent = ?", array($id));



				foreach ($_POST['price_attrs']['id_color'] as $k => $v) {

					if (!empty($_POST['price_attrs']['id_color'][$k]) && !empty($_POST['price_attrs']['id_size'][$k]) && !empty($_POST['price_attrs']['regular_price'][$k])) {

						$price_attrs['id_color'] = $_POST['price_attrs']['id_color'][$k];

						$price_attrs['id_size'] = $_POST['price_attrs']['id_size'][$k];

						$price_attrs['regular_price'] = (!empty($_POST['price_attrs']['regular_price'][$k])) ? str_replace(",", "", $_POST['price_attrs']['regular_price'][$k]) : 0;

						$d->insert('product_sale', $price_attrs);
					}
				}
			}
		...
		if ($d->insert('product', $data)) {
			...
			if (!empty($_POST['price_attrs'])) {
				...
				$price_attrs['id_parent'] = $id_insert;

				$d->rawQuery("delete from #_product_sale where id_parent = ?", array($id_insert));
				...
				foreach ($_POST['price_attrs']['id_color'] as $k => $v) {
					...
					if (!empty($_POST['price_attrs']['id_color'][$k]) && !empty($_POST['price_attrs']['id_size'][$k]) && !empty($_POST['price_attrs']['regular_price'][$k])) {
						...
						$d->insert('product_price', $price_attrs);
					}
				}
			}
