...
'script' => '@<script[^>]*?>.*?</script>@si',
	...
	public function getColor($id = 0)
	{
		...
		$str = '<select id="dataColor" name="id_color[]"  class="select_tt select multiselect multiselect-color" multiple="multiple" >';
	...
	public function getSize($id = 0)
	{
		...
		$str = '<select id="dataSize" name="id_size[]" class="select multiselect multiselect-size" multiple="multiple" >';
	...
	/* Get tags sale*/
	public function getSale($id = 0, $element = '', $table = '', $type = '')
	{
		if ($id) {
			$temps = $this->d->rawQuery("select id_sale from #_" . $table . " where id = ?", array($id));
			$temps = (!empty($temps)) ? $this->joinCols($temps, 'id_sale') : array();
			$temps = (!empty($temps)) ? explode(",", $temps) : array();
		}

		$row_sale = $this->cache->get("select namevi, id from #_news where type = ? order by numb,id desc", array($type), "result", 7200);

		$str = '<select id="' . $element . '" name="' . $element . '[]" class="select multiselect" multiple="multiple" >';
		for ($i = 0; $i < count($row_sale); $i++) {
			if (!empty($temps)) {
				if (in_array($row_sale[$i]['id'], $temps)) $selected = 'selected="selected"';
				else $selected = '';
			} else {
				$selected = '';
			}
			$str .= '<option value="' . $row_sale[$i]["id"] . '" ' . $selected . ' /> ' . $row_sale[$i]["namevi"] . '</option>';
		}
		$str .= '</select>';
	...
	public function getAjaxPlace($table = '', $title_select = 'Chọn danh mục')
	{
		...
		if ($table == 'city') {
			...
			$idcity = (isset($_REQUEST['id_city'])) ? htmlspecialchars($_REQUEST['id_citangy']) : 0;
			$where .= ' and id_city = ?';
	...
	public function checkActive($c, $array)
	{
		if ($array) {
			if (in_array($c, $array)) {
				return 1;
	...
	public function stringBrand($lang = 'vi')
	{
		$brand = array();
		$row = $this->cache->get("select name$lang from #_product_brand where type='san-pham' order by numb,id desc", null, "result", 7200);

		foreach ($row as $v) {
			$brand[] = substr($v['name' . $lang], 0, 1);
		}
	...
	public function headLike($id = 0)
	{
		global $loginMember;

		if (!empty($_SESSION[$loginMember]) || !empty($_COOKIE['login_member_id'])) {
			$iduser = (!empty($_COOKIE['login_member_id'])) ? $_COOKIE['login_member_id'] : $_SESSION[$loginMember]['id'];
			$row = $this->cache->get("select * from #_member_like where id_user=$iduser and id_product = $id limit 0,1", null, "fetch", 7200);
			if (isset($row['id']) && $row['id'] > 0) {
				$string =  '<i class="fas fa-heart"></i>';
			} else {
				$string = '<i class="far fa-heart"></i>';
			}
		} else {
			$string = '<i class="far fa-heart"></i>';
		}
	...
	public function sale($price_news = 0, $price_old = 0)
	{
		if ($price_old > 0) {
			$kq = '<span class="sale">' . round(100 - (($price_news / $price_old) * 100), 0) . '%</span>';
			return $kq;
		}
	}


	public function flashSale($time = 0, $time_out = 0)
	{
		$star_time = $this->d->rawQueryOne("select id, time_from, time_to from #_news where type = ? and time_from <= " . $time . " and  time_to >= " . $time . " and find_in_set('hienthi',status)", array('tags-sale'));
		if ($time >= $time_out) {
			$time1 =  $time_out * 60;
			$time2 =   $time - $star_time['time_from'];
			$t =  round(($time2 / $time1) * 100, 0);
			return $t;
		}
	}
}