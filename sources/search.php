...
if (!empty($_GET['keyword'])) {
	...
	if ($tukhoa) {
		$where = "";
		$where = "type = ? and time=0 and (name$lang LIKE ? or slugvi LIKE ? or slugen LIKE ?) and find_in_set('hienthi',status)";
		...
		$sql = "select photo, name$lang, slugvi, slugen, old_price, regular_price, id from #_product where $where order by numb,id desc $limit";
