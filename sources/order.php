...
if (!empty($_POST['thanhtoan'])) {
	...
	for ($i = 0; $i < $max; $i++) {
		...
		$pro_price = $cart->sumPrice($pid, $color, $size, $q);
		$text_attr = '';
		...
		$orderDetailVals = array(
			$proinfo['name' . $lang],
			$text_attr,
			$func->formatMoney($cart->sumPrice($pid, $color, $size)),
			$func->formatMoney($cart->sumPrice($pid, $color, $size)),
			$q,
	...
	if ($id_insert) {
		for ($i = 0; $i < $max; $i++) {
			...
			$code_order = $_SESSION['cart'][$i]['code'];
			$pro_price = $cart->sumPrice($pid, $_SESSION['cart'][$i]['color'], $_SESSION['cart'][$i]['size']);
			...
			$data_donhangchitiet['regular_price'] = $pro_price;
