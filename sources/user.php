...
switch ($action) {
	...
	case 'san-pham-thich':
		$titleMain = sanphamuathich;
		$template = "account/like";
		if (empty($_SESSION[$loginMember]['active'])) $func->transfer("Trang không tồn tại", $configBase, false);
		infoLike();
		break;
...
function infoLike()
{
	global $d, $func, $loginMember, $paging, $getPage, $url, $product_like, $lang;
	$type = 'san-pham';
	$array_pro = array();
	$rowPro = $d->rawQuery("select id_product, id_user, id from #_member_like where id_user = ?", array($_SESSION[$loginMember]['id']));
	foreach ($rowPro as $value) {
		$array_pro[] = $value['id_product'];
	}

	$string = implode(',', $array_pro);
	if ($string != '') {
		/* Lấy tất cả sản phẩm */
		$where = "";
		$where = "type = ? and find_in_set('hienthi',status) and id IN ($string)";
		$params = array($type);
		...
		$sql = "select photo, name$lang, slugvi, slugen, old_price, regular_price, discount, id from #_product where $where order by numb,id desc";
		$product_like = $d->rawQuery($sql, $params);
		// $func->dump($product);
