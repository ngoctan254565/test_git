...
if ($id != '') {
	/* Lấy sản phẩm detail */
	$rowDetail = $d->rawQueryOne("select type, id, name$lang, slugvi, slugen, desc$lang, content$lang, code, view, id_brand, id_list, id_cat, id_item, id_sub, photo, options, discount, old_price, regular_price from #_product where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($id, $type));
	...
	$sql = "select photo, name$lang, slugvi, slugen, old_price, regular_price, discount, id, desc$lang from #_product where $where order by numb,id desc $limit";
	...
	/* Lấy cấp 1 detail */
	$productList = $d->rawQueryOne("select id, name$lang, slugvi, slugen, type, photo, options, status from #_product_list where id = ? and type = ? limit 0,1", array($idl, $type));
	...
	/* Lấy sản phẩm */
	$where = "";
	$where = "id_list = ? and type = ?  $sql_search $where_sale and find_in_set('hienthi',status)";
	...
	$startpoint = ($curPage * $perPage) - $perPage;
	$limit = " limit " . $startpoint . "," . $perPage;
	$sql = "select photo, name$lang, slugvi, slugen, old_price, regular_price, discount, id, time, id_sale, desc$lang from #_product where $where  order by numb,id desc $limit";
	$product = $d->rawQuery($sql, $params);
	$sqlNum = "select count(*) as 'num' from #_product where $where order by numb,id desc";
	$count = $d->rawQueryOne($sqlNum, $params);
	$total = (!empty($count)) ? $count['num'] : 0;
	...
	/* Lấy sản phẩm */
	$where = "";
	$where = "id_cat = ? and type = ? $sql_search and find_in_set('hienthi',status)";
	...
	/* Lấy sản phẩm */
	$where = "";
	$where = "id_item = ? and type = ? $sql_search and find_in_set('hienthi',status)";
	...
	/* Lấy sản phẩm */
	$where = "";
	$where = "id_sub = ? and type = ? $sql_search and find_in_set('hienthi',status)";
	...
	/* Lấy sản phẩm */
	$where = "";
	$where = "id_brand = ? and type = ? $sql_search and time=0 and find_in_set('hienthi',status)";
	...
	$startpoint = ($curPage * $perPage) - $perPage;
	$limit = " limit " . $startpoint . "," . $perPage;
	$sql = "select photo, name$lang, slugvi, slugen, old_price, regular_price, discount, id, time, id_sale from #_product where $where order by numb,id desc $limit";
	$product = $d->rawQuery($sql, $params);
	$sqlNum = "select count(*) as 'num' from #_product where $where order by numb,id desc";
	$count = $d->rawQueryOne($sqlNum, $params);
	$total = (!empty($count)) ? $count['num'] : 0;
	...
	/* Lấy tất cả sản phẩm */
	$where = "";
	$where = "type = ? $sql_search and time=0 and find_in_set('hienthi',status)";
	$params = array($type);
