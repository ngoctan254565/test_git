...
$banner = $cache->get("select id, photo, options from #_photo where type = ? and act = ? limit 0,1", array('banner', 'photo_static'), 'fetch', 7200);
$imgvs = $cache->get("select photo from #_photo where type = ? and act = ? limit 0,1", array('photo-vs', 'photo_static'), 'fetch', 7200);
...
$splist = $cache->get("select name$lang, slugvi, slugen, id from #_product_list where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('cac-dong-xe'), 'result', 7200);
$vllist = $cache->get("select name$lang, slugvi, slugen, id from #_news_list where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('vat-lieu-xay-dung'), 'result', 7200);
$svlist = $cache->get("select name$lang, slugvi, slugen, id from #_news_list where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('dich-vu'), 'result', 7200);
...
$slider1 = $cache->get("select name$lang, photo, desc$lang, link from #_photo where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('slide1'), 'result', 7200);
